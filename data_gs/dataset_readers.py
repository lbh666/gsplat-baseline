...
from data_gs.colmap_loader import read_extrinsics_text, read_intrinsics_text, qvec2rotmat, \
...
def getNerfppNorm(cam_info):
    def get_center_and_diag(cam_centers):
        ...
        dist = np.abs(cam_centers - center)
...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder) -> list[CameraInfo]:
    ...
    for idx, key in enumerate(cam_extrinsics):
        ...
        fid = int(image_name[-5:]) / (num_frames - 1)
...
def readDTUCameras(path, render_camera, object_camera) -> list[CameraInfo]:
...
def readNerfiesCameras(path) -> list[CameraInfo]:
...
def readCamerasFromNpy(path, npy_file, split, hold_id, num_images) -> list[CameraInfo]:
    cam_infos = []
